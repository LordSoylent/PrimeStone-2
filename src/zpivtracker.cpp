...
CzPSCTracker::CzPSCTracker(std::string strWalletFile)
{
...
CzPSCTracker::~CzPSCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzPSCTracker::Init()
{
...
bool CzPSCTracker::Archive(CMintMeta& meta)
{
...
bool CzPSCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzPSCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzPSCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzPSCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzPSCTracker::GetSerialHashes()
{
...
CAmount CzPSCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzPSCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzPSCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzPSCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzPSCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzPSCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzPSCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzPSCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzPSCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzPSCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzPSCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzPSCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzPSCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzPSCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzPSCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzPSCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
