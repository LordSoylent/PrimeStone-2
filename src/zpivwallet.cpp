...
CzPSCWallet::CzPSCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zPSC seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadzPSCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.ErasezPSCSeed_deprecated()) {
                    LogPrintf("%s: Updated zPSC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zPSC seed\n", __func__);
                }
            }
        }
    }
...
bool CzPSCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WritezPSCCount(nCountLastUsed);
    else if (!walletdb.ReadzPSCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzPSCWallet::Lock()
{
    seedMaster = 0;
}

void CzPSCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzPSCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedTozPSC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzPSCWallet::LoadMintPoolFromDB()
{
...
void CzPSCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzPSCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzPSCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zPSCTracker->HasPubcoinHash(pMint.first)) {
...
bool CzPSCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedTozPSC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zPSCTracker which also adds to database
    pwalletMain->zPSCTracker->Add(dMint, true);
...
void CzPSCWallet::SeedTozPSC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzPSCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzPSCWallet::GenerateDeterministiczPSC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzPSCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzPSCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
