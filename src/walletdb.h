...
public:
    ...
    bool WritezPSCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadzPSCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadzPSCSeed_deprecated(uint256& seed);
    bool ErasezPSCSeed();
    bool ErasezPSCSeed_deprecated();

    bool WritezPSCCount(const uint32_t& nCount);
    bool ReadzPSCCount(uint32_t& nCount);
